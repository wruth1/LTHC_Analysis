...
using JLD2
using LinearAlgebra # For eigvals() & inverse and diag functions
...
data_from_R = load("data/all_Y_trajectories.RData")


all_Y_imputations = data_from_R["all_Ys"]

Y_vec_raw = all_Y_imputations[job_id]
...
all_phi_0_inits = [1.0, 3.0]
all_gamma_inits = [0.1]
all_lambda_inits = [0.1]
...
# ------------------------------- Run algorithm ------------------------------ #
Random.seed!(1)
Threads.@threads for i in eachindex(all_theta_inits)
    println("\n Iteration $i")
    Random.seed!(i^2)
    # Random.seed!(i^3)
...
# using JLD2
# @save "output/sim_results/Truncated_MCEM_Results-tol=1e-3,jobid=$job_id.jld2" all_theta_hat_trajectories_MCEM all_theta_hats_MCEM all_cov_hats_MCEM all_Ms_MCEM all_num_iterations_MCEM
# @save "output/Truncated_MCEM_Results-No_Singles.jld2" all_theta_hat_trajectories_MCEM all_theta_hats_MCEM all_cov_hats_MCEM all_Ms_MCEM all_num_iterations_MCEM

# # Load results from file
# using JLD2
# @load "output/Truncated_MCEM_Results.jld2" all_theta_hat_trajectories_MCEM all_theta_hats_MCEM all_cov_hats_MCEM all_Ms_MCEM all_num_iterations_MCEM
...
# save("output/Truncated_MCEM_Estimate-tol=1e-3,jobid=$job_id.jld2", "final_theta_hat_MCEM", final_theta_hat_MCEM)
# save("output/Truncated_MCEM_Estimate-No_Singles.jld2", "final_theta_hat_MCEM", final_theta_hat_MCEM)
...
# # Save data
# # using CSV
# CSV.write("output/Truncated_Results-tol=1e-3,jobid=$job_id.csv", MCEM_data)
# CSV.write("output/Truncated_Results-No_Singles.csv", MCEM_data)

# # Load data
# MCEM_data = CSV.read("output/Truncated_Results.csv", DataFrame)
...
theta_cov = all_cov_hats_MCEM[ind_max]
...
# CSV.write("output/Truncated_Estimate_Analysis-tol=1e-3,jobid=$job_id.csv", data_theta_hat)
# CSV.write("output/Truncated_Estimate_Analysis-No_Singles.csv", data_theta_hat)
...
inds_close = findall(x->x>=max_log_LR-SE_log_LR, log_LR_estimates)
all_theta_hats_close = all_theta_hats_MCEM[inds_close]
all_cov_hats_close = all_cov_hats_MCEM[inds_close]
...
# savefig(full_plot, "Plots/Truncated_Trajectories-tol=1e-3.pdf")
# savefig(full_plot, "Plots/Truncated_Trajectories-No_Singles.pdf")
...
function get_R0_hat(theta_hat)
    return theta_hat[1] / theta_hat[3]
end

function get_SE_R0_hat(theta_hat, theta_cov)
    A = 1 / theta_hat[3]
    B = -theta_hat[1] / theta_hat[3]^2
    jacob = [A, B]

    Sigma_13 = theta_cov[[1,3], [1,3]]

    return sqrt(jacob' * Sigma_13 * jacob)
end


R0_hat = get_R0_hat(theta_hat)
R0_SE = get_SE_R0_hat(theta_hat, theta_cov)


all_R0s_close = get_R0_hat.(all_theta_hats_close)
all_R0_SEs_close = get_SE_R0_hat.(all_theta_hats_close, all_cov_hats_close)
...
@save "output/sim_results/sim_results-$job_id.jld2" all_theta_hat_trajectories_MCEM all_theta_hats_MCEM all_cov_hats_MCEM all_Ms_MCEM all_num_iterations_MCEM

@save "output/estimates_short/estimates_short-$job_id.jld2" theta_hat theta_cov R0_hat R0_SE

@save "output/estimates_long/estimates_long-$job_id.jld2" all_theta_hats_close all_cov_hats_close all_R0s_close all_R0_SEs_close