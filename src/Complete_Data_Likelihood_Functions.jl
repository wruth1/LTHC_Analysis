...
function complete_data_log_lik_one_Y(theta, Y, X, omega_traj)
    ...
    log_lik_obs = 0
    for t in 1:t_max
        this_lik_term = one_day_log_lik(t, Y, X, omega_traj, phi_0, gamma, lambda)
        if isinf(this_lik_term)
            println("Infinite likelihood term at t = $t")
        end
        # println(this_lik_term)
        log_lik_obs += this_lik_term
    end
